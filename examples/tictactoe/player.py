...
class TicTacToePlayer(OmniBase, Agent, GamePlayerMixin):
    ...
    def _random_strategy(self, board_state: list) -> tuple:
        ...
        empty_spots = [(r, c) for r in range(3)
                       for c in range(3) if board_state[r][c] == " "]
        ...
        for r in range(3):
            for c in range(3):
                if board_state[r][c] == " ":
                    board_state[r][c] = my_symbol
                    score = self._minimax(
                        board_state, 0, False, my_symbol, opponent_symbol)
                    board_state[r][c] = " "
                    if score > best_score:
                        best_score = score
                        best_move = (r, c)
            ...
            for r in range(3):
                for c in range(3):
                    if board[r][c] == " ":
                        board[r][c] = my_symbol
                        score = self._minimax(
                            board, depth + 1, False, my_symbol, opponent_symbol)
                        board[r][c] = " "
            ...
            for r in range(3):
                for c in range(3):
                    if board[r][c] == " ":
                        ...
                        score = self._minimax(
                            board, depth + 1, True, my_symbol, opponent_symbol)
                        board[r][c] = " "
    ...
    async def _try_make_move(self):
        ...
        self.logger.info(
            f"Player {self.player_id} thinking about move in game {self.current_game_id}")
    ...
    def assign_to_game(self, game_id: str, symbol: str):
        ...
        self.logger.debug(
            f"Player {self.player_id} assigned to game {game_id} as {symbol}")
    ...
    def get_widget(self, asset: str = "default", **kwargs) -> str:
        ...
        games_played = self.games_played or 0
        win_rate = total_wins / \
            max(1, games_played) if games_played > 0 else 0.0
        ...
        for r in range(3):
            for c in range(3):
                ...
                if cell_val != " ":
                    ...
                    color = (255, 100, 100) if cell_val == "X" else (
                        100, 100, 255)
                    if font:
