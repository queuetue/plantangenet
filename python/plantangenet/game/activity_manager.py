...
class ActivityManager(Runner):
    """
    Generic activity manager for running, stepping, or updating a collection of agents, omnis, or activities.
    ...
    def broadcast_event(self, event, *args, **kwargs):
        for activity in self.activities:
            if hasattr(activity, 'on_event'):
                activity.on_event(event, *args, **kwargs)
